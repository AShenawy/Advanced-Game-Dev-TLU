...
public class UIGoblin : MonoBehaviour, IPointerClickHandler, IDragHandler, IBeginDragHandler, IEndDragHandler, IDropHandler
{
    ...
    // Important! This script is called not called on the object being dragged, but the object the drag is stopped on
    // So if we dragged ObjA and dropped it on ObjB, OnEndDrag (if used) is called on ObjA & OnDrop (if used) is called on ObjB,
